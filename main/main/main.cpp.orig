...
extern "C" void app_main() {
    ...
    // Initialize ESP32 pins for input/output
    GPIO_Init(input);
    GPIO_Init(output);
...
=======
    // Enable mux, decoder on/off pins
    gpio_set_level(decoderEn, 1);
    gpio_set_level(muxEn_n, 0);
    
    pixels.begin();       // Control LED strip
    Chess::Board board(); // Create board object
    
    while(1) {
        //delay(1000);
        Scan_Hall();     // Read piece placement from hall sensors
        Serial_Output(); // Output findings to console
        //LedStrip_Output();
    }
>>>>>>> 11eccccdac265ed099ecb2cd91388771a036a9cf
}

IOController::Move HalController.detectChange(void) {
